...
- **프론트엔드**: Vercel → https://chat.sonamu.my
- **백엔드**: service1과 같은 AWS Lightsail 인스턴스에 systemd 서비스(`service2-backend`)로 상시 실행, nginx + Let's Encrypt로 https://chat.sonamu.my/api 서빙. `SERVICE1_API_URL`은 같은 서버 내부 통신이라 `http://localhost:4000/api` 사용.
